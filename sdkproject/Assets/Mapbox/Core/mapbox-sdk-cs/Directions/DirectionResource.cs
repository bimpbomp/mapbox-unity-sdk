...
namespace Mapbox.Directions
{
	...
	{
		// Optional
		private readonly string apiEndpoint = "directions/v5/";
		...
		public override string GetUrl()
		{
			...
			if (Alternatives != null) opts.Add("alternatives", Alternatives.ToString().ToLower());

			if (Bearings != null) opts.Add("bearings", GetUrlQueryFromArray(Bearings, ";"));

			if (ContinueStraight != null) opts.Add("continue_straight", ContinueStraight.ToString().ToLower());

			if (Overview != null) opts.Add("overview", Overview.ToString());

			if (Radiuses != null) opts.Add("radiuses", GetUrlQueryFromArray(Radiuses));

			if (Steps != null) opts.Add("steps", Steps.ToString().ToLower());

			return Constants.BaseAPI +
			       ApiEndpoint +
			       RoutingProfile +
			       GetUrlQueryFromArray(Coordinates, ";") +
			       ".json" +
