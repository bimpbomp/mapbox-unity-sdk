...
namespace Mapbox.Geocoding
{
	...
	public sealed class ForwardGeocodeResource : GeocodeResource<string>
	{
		...
		public override string GetUrl()
		{
			...
			if (Autocomplete != null) opts.Add("autocomplete", Autocomplete.ToString().ToLower());

			if (Bbox != null)
			{
				var nonNullableBbox = (Vector2dBounds)Bbox;
				opts.Add("bbox", nonNullableBbox.ToString());
			}

			if (Country != null) opts.Add("country", GetUrlQueryFromArray(Country));

			if (Proximity != null)
			{
				var nonNullableProx = (Vector2d)Proximity;
