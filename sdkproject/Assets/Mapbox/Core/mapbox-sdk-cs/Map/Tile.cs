...
namespace Mapbox.Map
{
	...
	///     <see href="https://en.wikipedia.org/wiki/Tiled_web_map">
	///         here
	///     </see>
	///     .
	/// </summary>
	public abstract class Tile : IAsyncRequest
	{
		...
		public bool IsCompleted => CurrentState == State.Loaded;
		...
		public void Initialize(Parameters param, Action callback)
		{
			...
			_id = param.Id;
			_callback = callback;
			_request = param.Fs.Request(MakeTileResource(param.TilesetId).GetUrl(), HandleTileResponse, tileId: _id,
		...
		internal void Initialize(IFileSource fileSource, CanonicalTileId canonicalTileId, string tilesetId, Action p)
		{
			...
			_id = canonicalTileId;
			_callback = p;
			_request = fileSource.Request(MakeTileResource(tilesetId).GetUrl(), HandleTileResponse, tileId: _id,
				tilesetId: tilesetId);
		}
