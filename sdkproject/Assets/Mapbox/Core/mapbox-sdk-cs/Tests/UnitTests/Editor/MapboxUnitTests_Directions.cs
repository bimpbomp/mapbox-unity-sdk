...
namespace Mapbox.MapboxSdkCs.UnitTest
{
	...
	{
		private readonly string _basicResponse =
		...
		private readonly string _responseWithSteps =
		...
		private readonly Directions.Directions _directions = new(new FileSource(
			MapboxAccess.Instance.Configuration.GetMapsSkuToken, MapboxAccess.Instance.Configuration.AccessToken));
		...
		public void SerializesAndDeserializesBasic()
		{
			...
			var basicResp = _directions.Deserialize(_basicResponse);

			// Then deserialize it back to a string.
			var basicReserialized = _directions.Serialize(basicResp);
		...
		{
			// First, deserialize the example response.
			var withStepsResp = _directions.Deserialize(_responseWithSteps);

			// Then deserialize it back to a string.
			var withStepsReserialized = _directions.Serialize(withStepsResp);
