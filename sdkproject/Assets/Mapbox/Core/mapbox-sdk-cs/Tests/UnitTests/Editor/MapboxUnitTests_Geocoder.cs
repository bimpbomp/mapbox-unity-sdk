...
namespace Mapbox.MapboxSdkCs.UnitTest
{
	...
	{
		private readonly Geocoder _geocoder = new(new FileSource(MapboxAccess.Instance.Configuration.GetMapsSkuToken,
			MapboxAccess.Instance.Configuration.AccessToken)); //MapboxAccess.Instance.Geocoder;

		private readonly string _forwardResponse =
		...
		private readonly string _reverseResponse =
		...
		public void SerializesAndDeserializesReverse()
		{
			...
			var reverseResp = _geocoder.Deserialize<ReverseGeocodeResponse>(_reverseResponse);

			// Then deserialize it back to a string.
			var reverseReserialized = JsonConvert.SerializeObject(reverseResp, JsonConverters.Converters);
		...
		{
			// First, deserialize the example response
			var forwardResp = _geocoder.Deserialize<ForwardGeocodeResponse>(_forwardResponse);

			// Then deserialize it back to a string.
			var forwardReserialized = JsonConvert.SerializeObject(forwardResp, JsonConverters.Converters);
