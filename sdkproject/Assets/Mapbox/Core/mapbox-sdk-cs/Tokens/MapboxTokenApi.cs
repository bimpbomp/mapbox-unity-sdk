...
namespace Mapbox.Tokens
{
	...
	public class MapboxTokenApi
	{
		...
		{
			if (_fs == null) _fs = new FileSource(skuToken);

			_fs.Request(
				Constants.BaseAPI + "tokens/v2?access_token=" + accessToken,
				response =>
				{
					if (response.HasError)
					{
						callback(new MapboxToken { HasError = true, ErrorMessage = response.ExceptionsAsString });
						return;
					}
