...
namespace Mapbox.Examples.Scripts
{
	public class LogLocationProviderData : MonoBehaviour
	{
		...
		private LocationLogWriter _logWriter;


		private void Start()
		{
			Screen.sleepTimeout = SleepTimeout.NeverSleep;
			LocationProviderFactory.Instance.DefaultLocationProvider.OnLocationUpdated +=
			...
			if (null == _logText) Debug.LogError("no text to log to");
		}
		...
		private void LocationProvider_OnLocationUpdated(Location location)
		{
			...
			var locationProviderClass = LocationProviderFactory.Instance.DefaultLocationProvider.GetType().Name;
			sb.AppendLine(string.Format("location provider: {0} ({1})", location.Provider, locationProviderClass));
			sb.AppendLine(string.Format("UTC time:{0}  - device:  {1}{0}  - location:{2}", Environment.NewLine,
				DateTime.UtcNow.ToString("yyyyMMdd HHmmss"),
				UnixTimestampUtils.From(location.Timestamp).ToString("yyyyMMdd HHmmss")));
			sb.AppendLine(string.Format(_invariantCulture, "position: {0:0.00000000} / {1:0.00000000}",
				location.LatitudeLongitude.x, location.LatitudeLongitude.y));
			...
			sb.AppendLine(nullableAsStr(location.SatellitesUsed, "SatellitesUsed:{0} "));
			sb.AppendLine(nullableAsStr(location.SatellitesInView, "SatellitesInView:{0}"));

			if (null != _logText) _logText.text = sb.ToString();
			...
			if (_logToFile && null != _logWriter) _logWriter.Write(location);
		}
