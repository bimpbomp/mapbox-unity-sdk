...
namespace Mapbox.Examples.Playground
{
	public class VectorTileExample : MonoBehaviour, IObserver<Map.VectorTile>
	{
		...
		private void Start()
		{
			_map = new Map<Map.VectorTile>(new FileSource(MapboxAccess.Instance.Configuration.GetMapsSkuToken,
		...
		{
			if (tile.CurrentState != Tile.State.Loaded || tile.HasError) return;
			...
			var sub = data.Length < 5000 ? data : data.Substring(0, 5000) + "\n. . . ";
