...
{
	internal class VoxelTile : MonoBehaviour, IObserver<RasterTile>, IObserver<RawPngRasterTile>
	{
		[SerializeField] private ForwardGeocodeUserInput _geocodeInput;

		[SerializeField] private int _zoom = 17;

		[SerializeField] private float _elevationMultiplier = 1f;

		[SerializeField] private int _voxelDepthPadding = 1;

		[SerializeField] private int _tileWidthInVoxels;

		[SerializeField] private VoxelFetcher _voxelFetcher;

		[SerializeField] private GameObject _camera;

		[SerializeField] private int _voxelBatchCount = 100;
		...
		private readonly List<GameObject> _instantiatedVoxels = new();
		...
		private readonly List<VoxelData> _voxels = new();
		...
		private void Start()
		{
			...
			if (!string.IsNullOrEmpty(_styleUrl)) _raster.TilesetId = _styleUrl;
		...
		{
			if (_geocodeInput) _geocodeInput.OnGeocoderResponse -= GeocodeInput_OnGeocoderResponse;
		}
		...
		public void OnNext(RasterTile tile)
		{
			...
			)
			{
				...
				if (ShouldBuildWorld()) BuildVoxelWorld();
			}
		}
		...
		private void Cleanup()
		{
			...
			_voxels.Clear();
			foreach (var voxel in _instantiatedVoxels) voxel.Destroy();
		}
		...
		private void BuildVoxelWorld()
		{
			...
			for (var x = 0; x < _rasterTexture.width; x++)
			for (var z = 0; z < _rasterTexture.height; z++)
			{
				...
				for (var y = startHeight; y < height; y++)
			...
			if (this != null) StartCoroutine(BuildRoutine());
		}

		private IEnumerator BuildRoutine()
		{
			var distanceOrderedVoxels =
				_voxels.OrderBy(x => (_camera.transform.position - x.Position).magnitude).ToList();

			for (var i = 0; i < distanceOrderedVoxels.Count; i += _voxelBatchCount)
			{
				for (var j = 0; j < _voxelBatchCount; j++)
				{
					var index = i + j;
					if (index < distanceOrderedVoxels.Count)
					{
						var voxel = distanceOrderedVoxels[index];
						_instantiatedVoxels.Add(Instantiate(voxel.Prefab, voxel.Position, Quaternion.identity,
							transform));
					}
				}
