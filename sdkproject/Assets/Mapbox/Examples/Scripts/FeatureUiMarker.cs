...
namespace Mapbox.Examples
{
	...
	{
		[SerializeField] private Transform _wrapperMarker;
		...
		private void Snap()
		{
			...
			{
				var pos = Camera.main.WorldToScreenPoint(_selectedFeature.Transform.position +
				                                         _selectedFeature.Transform.lossyScale.x * vert);
				if (pos.x < left)
					left = pos.x;
				else if (pos.x > right)
					right = pos.x;
			...
			_info.text = string.Join(" \r\n ",
				_selectedFeature.Feature.Properties.Select(x => x.Key + " - " + x.Value).ToArray());
		}
	}
}
