...
namespace Mapbox.Examples
{
	public class RotateWithLocationProvider : MonoBehaviour
	{
		...
		[SerializeField]
		[Tooltip(
			"Per default 'UserHeading' (direction the device is moving) is used for rotation. Check to use 'DeviceOrientation' (where the device is facing)")]
		...
		[SerializeField]
		[Tooltip(
			"Set this to true if you'd like to adjust the sign of the rotation angle. eg angle passed in 63.5, angle that should be used for rotation: -63.5.")]
		...
		{
			if (LocationProvider != null) LocationProvider.OnLocationUpdated -= LocationProvider_OnLocationUpdated;
		}

		private void LocationProvider_OnLocationUpdated(Location location)
		{
			var rotationAngle = _useDeviceOrientation ? location.DeviceOrientation : location.UserHeading;
			...
			else
			{
				...
				if (location.IsUserHeadingUpdated) _targetRotation = Quaternion.Euler(getNewEulerAngles(rotationAngle));
			}
		}
		...
		private Vector3 getNewEulerAngles(float newAngle)
		{
			...
			var euler = Constants.Math.Vector3Zero;
