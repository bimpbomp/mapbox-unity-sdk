...
namespace Mapbox.Unity.Map
{
	...
	public class TileJsonData
	{
		...
		{
			tileJSONLoaded = true;
			var layerPropertiesList = new List<string>();

			if (tjr == null || tjr.VectorLayers == null || tjr.VectorLayers.Length == 0) return;
			...
			foreach (var layer in tjr.VectorLayers)
			{
				...
				if (SourceLayersDictionary.ContainsKey(layerSource))
				{
					    ...
					    sourceList.Contains(layerSource)) // the current layerName has more than one source
					{
						...
						if (LayerDisplayNames.Contains(layerName)) LayerDisplayNames.Remove(layerName);
				...
				foreach (var property in layer.Fields)
				{
					...
					{
						LayerPropertyDescriptionDictionary.Add(layerName,
							new Dictionary<string, string> { { propertyName, propertyDescription } });
					}
					...
					if (PropertyDisplayNames.ContainsKey(layerName))
					{
						if (!PropertyDisplayNames[layerName].Contains(propertyName))
						{
							...
							{
								var firstSource = tjr.Source.Split(new[] { "," }, StringSplitOptions.None)[0].Trim();
			...
			if (LayerDisplayNames.Count > 1) LayerDisplayNames.Sort();
		}
	}
}
