...
namespace Mapbox.Editor
{
	public class FeaturesSubLayerPropertiesDrawer
	{
		...
		private readonly BehaviorModifiersSectionDrawer _behaviorModifierSectionDrawer = new();
		private bool _isGUIContentSet;
		private bool _isInitialized;
		private int _layerIndex;
		private GUIContent[] _layerTypeContent;

		private readonly ModelingSectionDrawer _modelingSectionDrawer = new();
		...
		public void DrawUI(SerializedProperty property)
		{
			...
			switch (sourceTypeValue)
			{
				...
				case VectorSourceType.Custom:
					if (_isInitialized)
					{
						var test = layerSourceId.stringValue;
						LoadEditorTileJSON(property, sourceTypeValue, layerSourceId.stringValue);
					}
					else
					{
			...
			if (sourceTypeValue != VectorSourceType.None)
			{
				...
				{
					text = "Map Features", tooltip = "Visualizers for vector features contained in a layer. "
				});

				var subLayerArray = property.FindPropertyRelative("vectorSubLayers");

				var layersRect = EditorGUILayout.GetControlRect(
					GUILayout.MinHeight(Mathf.Max(subLayerArray.arraySize + 1, 1) * _lineHeight +
				...
				if (EditorGUILayout.DropdownButton(new GUIContent { text = "Add Feature" }, FocusType.Passive,
					    "minibuttonleft")) menu.DropDown(rect);
		...
		{
			var featureType = (PresetFeatureType)Enum.Parse(typeof(PresetFeatureType), name.ToString());
		...
		private void SetSubLayerProps(SerializedProperty subLayer)
		{
			...
			var coreOptions = subLayerProperties.coreOptions;
			...
			if (topMatArray.arraySize == 0) topMatArray.arraySize = 1;
			if (sideMatArray.arraySize == 0) sideMatArray.arraySize = 1;
			...
			if (customTopMatArray.arraySize == 0) customTopMatArray.arraySize = 1;
			if (customSideMatArray.arraySize == 0) customSideMatArray.arraySize = 1;
			...
			string sourceString)
		{
			if (sourceTypeValue != VectorSourceType.None && !string.IsNullOrEmpty(sourceString))
			{
				if (tileJSONResponse == null || string.IsNullOrEmpty(sourceString) || sourceString != TilesetId)
					try
					{
						MapboxAccess.Instance.TileJSON.Get(sourceString, response =>
						{
		...
		public void DrawLayerName(SerializedProperty property, List<string> layerDisplayNames)
		{
			...
			if (layerDisplayNames.Count == 0)
			{
				EditorGUILayout.LabelField(layerNameLabel,
					new GUIContent("No layers found: Invalid TilesetId / No Internet."), "minipopUp");
				return;
			}
			...
			for (var extIdx = 0; extIdx < layerDisplayNames.Count; extIdx++)
			...
			var parsedString = layerDisplayNames.ToArray()[_layerIndex]
				.Split(new[] { tileJsonData.commonLayersKey }, StringSplitOptions.None)[0].Trim();
			...
			VectorSourceType sourceType)
		{
			...
			var roadLayer = layerProperty.FindPropertyRelative("roadLayer").stringValue;
			var landuseLayer = layerProperty.FindPropertyRelative("landuseLayer").stringValue;
			...
			if (visualizerLayer == roadLayer || visualizerLayer == landuseLayer)
			{
				...
				if (_streetsV7TileStats != null && _streetsV7TileStats.layers != null &&
					...
					{
						if (layer.layer != visualizerLayer) continue;

						var presetPropertyName = "";
