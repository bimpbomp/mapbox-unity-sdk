...
namespace Mapbox.Editor
{
	public class StyleIconBundle
	{
		...
		{
			path = Path.Combine(Constants.Path.MAP_FEATURE_STYLES_SAMPLES,
				Path.Combine(styleName, string.Format("{0}Icon", styleName + paletteName)));
		}

		public void Load()
		{
			if (texture == null) texture = Resources.Load(path) as Texture;
		}
	}
	...
	public class GeometryMaterialOptionsDrawer : PropertyDrawer
	{
		...
		private readonly Dictionary<SamplePalettes, StyleIconBundle> _paletteIconBundles = new()
		{
			{
		...
		private readonly Dictionary<StyleTypes, StyleIconBundle> _styleIconBundles = new()
		{
		...
		{
			foreach (var key in _styleIconBundles.Keys) _styleIconBundles[key].Load();
			foreach (var key in _paletteIconBundles.Keys) _paletteIconBundles[key].Load();
		}
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			showTexturing = EditorGUILayout.Foldout(showTexturing,
				new GUIContent
				{
			...
			if (showTexturing)
			{
				...
				var styleTypeGuiContent = new GUIContent[styleType.enumDisplayNames.Length];
				for (var i = 0; i < styleType.enumDisplayNames.Length; i++)
					styleTypeGuiContent[i] = new GUIContent { text = styleType.enumDisplayNames[i] };

				EditorGUI.BeginChangeCheck();
				styleType.enumValueIndex =
					EditorGUILayout.Popup(styleTypeLabel, styleType.enumValueIndex, styleTypeGuiContent);
				...
				if ((StyleTypes)styleType.enumValueIndex != StyleTypes.Custom)
				{
					...
					var thumbnailTexture = (Texture2D)_styleIconBundles[style].texture;
					...
					var descriptionLabel = style.Description();
					EditorGUILayout.LabelField(new GUIContent(" ", thumbnailTexture),
						Constants.GUI.Styles.EDITOR_TEXTURE_THUMBNAIL_STYLE, GUILayout.Height(60),
						GUILayout.Width(EditorGUIUtility.labelWidth - 60));
					EditorGUILayout.TextArea(descriptionLabel, "wordWrappedLabel");
					...
					switch ((StyleTypes)styleType.enumValueIndex)
					{
						case StyleTypes.Simple:
							...
							var samplePaletteTypeGuiContent = new GUIContent[samplePaletteType.enumDisplayNames.Length];
							for (var i = 0; i < samplePaletteType.enumDisplayNames.Length; i++)
								samplePaletteTypeGuiContent[i] = new GUIContent
								{
									text = samplePaletteType.enumDisplayNames[i]
								};
							samplePaletteType.enumValueIndex = EditorGUILayout.Popup(samplePaletteTypeLabel,
								samplePaletteType.enumValueIndex, samplePaletteTypeGuiContent);
							break;
						case StyleTypes.Light:
							property.FindPropertyRelative("lightStyleOpacity").floatValue =
								EditorGUILayout.Slider("Opacity",
									property.FindPropertyRelative("lightStyleOpacity").floatValue, 0.0f, 1.0f);
							break;
						case StyleTypes.Dark:
							property.FindPropertyRelative("darkStyleOpacity").floatValue =
								EditorGUILayout.Slider("Opacity",
									property.FindPropertyRelative("darkStyleOpacity").floatValue, 0.0f, 1.0f);
							break;
				...
				else
				{
					...
					var valIndex = texturingType.enumValueIndex == 0 ? 0 : texturingType.enumValueIndex + 1;
					var texturingTypeGUI = new GUIContent
					{
						text = "Texturing Type", tooltip = ((UvMapType)valIndex).Description()
					};
					...
					if (matList.arraySize == 0) matList.arraySize = 2;
					GUILayout.Space(-lineHeight);

					EditorGUI.BeginChangeCheck();
					EditorGUILayout.PropertyField(matList.GetArrayElementAtIndex(0),
						new GUIContent
						{
					...
					EditorGUI.BeginChangeCheck();
					EditorGUILayout.PropertyField(matList.GetArrayElementAtIndex(1),
						new GUIContent
						{
					...
					if ((UvMapType)texturingType.enumValueIndex + 1 == UvMapType.Atlas)
					{
						...
						EditorGUILayout.ObjectField(atlasInfo,
							new GUIContent
							{
								text = "Altas Info",
								tooltip =
					...
					if ((UvMapType)texturingType.enumValueIndex + 1 == UvMapType.AtlasWithColorPalette)
					{
						...
						EditorGUILayout.ObjectField(colorPalette,
							new GUIContent
							{
								text = "Color Palette",
								tooltip =
