...
public class ScriptablePaletteEditor : Editor
{
	...
	{
		var sp = (ScriptablePalette)target;

		EditorGUILayout.Space();

		var wrapTextStyle = new GUIStyle();
		wrapTextStyle.wordWrap = true;
		wrapTextStyle.normal.textColor =
			new Color(_HELPER_TEXT_COLOR_VAL, _HELPER_TEXT_COLOR_VAL, _HELPER_TEXT_COLOR_VAL, 1.0f);
		...
		var keyColorContent = new GUIContent("Key color", _KEY_COLOR_INFO);
		sp.m_keyColor = EditorGUILayout.ColorField(keyColorContent, sp.m_keyColor);

		EditorGUILayout.Space();

		var numColorContent = new GUIContent("Palette Size", _NUM_COLOR_INFO);
		sp.m_numColors = EditorGUILayout.IntSlider(numColorContent, sp.m_numColors, _NUM_COLORS_MIN, _NUM_COLORS_MAX);

		var hueRangeContent = new GUIContent("Hue Range", _HUE_RANGE_INFO);
		var satRangeContent = new GUIContent("Saturation Range", _SAT_RANGE_INFO);
		var valRangeContent = new GUIContent("Value Range", _VAL_RANGE_INFO);
		...
		if (GUILayout.Button("Generate Palette")) sp.GeneratePalette();
		...
		if (sp.m_colors == null || sp.m_colors.Length == 0)
			...
			for (var i = 0; i < sp.m_colors.Length; i++)
			{
				var fieldName = string.Format(_COLOR_FIELD_PREFIX, i);
		...
		EditorGUILayout.LabelField("Layer colors can be overriden by enabling any of the overrides below.",
			wrapTextStyle);
		EditorGUILayout.LabelField(
