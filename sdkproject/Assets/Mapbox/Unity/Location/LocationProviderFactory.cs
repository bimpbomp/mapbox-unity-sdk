...
namespace Mapbox.Unity.Location
{
	...
	public class LocationProviderFactory : MonoBehaviour
	{
		...
		[SerializeField] [Tooltip("Provider using Unity's builtin 'Input.Location' service")]
		private AbstractLocationProvider _deviceLocationProviderUnity;

		[SerializeField] [Tooltip("Custom native Android location provider. If this is not set above provider is used")]
		...
		{
			Debug.LogFormat("LocationProviderFactory: Injected EDITOR Location Provider - {0}",
		...
		{
			var AndroidApiVersion = 0;
			var regex = new Regex(@"(?<=API-)-?\d+");
			var match = regex.Match(SystemInfo
				.operatingSystem); // eg 'Android OS 8.1.0 / API-27 (OPM2.171019.029/4657601)'
			...
			{
				Debug.LogFormat("LocationProviderFactory: Injected native Android DEVICE Location Provider - {0}",
					_deviceLocationProviderAndroid.GetType());
				DefaultLocationProvider = _deviceLocationProviderAndroid;
			}
			else
			{
				Debug.LogFormat("LocationProviderFactory: Injected DEVICE Location Provider - {0}",
