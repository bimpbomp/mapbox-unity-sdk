...
namespace Mapbox.Unity.Map
{
	...
	///     Abstract map encapsulates the image, terrain and vector sources and provides a centralized interface to control the
	...
	public class AbstractMap : MonoBehaviour, IMap
	{
		...
		[SerializeField] private MapOptions _options = new();
		[SerializeField] private bool _initializeOnStart = true;
		[SerializeField] protected ImageryLayer _imagery = new();
		[SerializeField] protected TerrainLayer _terrain = new();
		[SerializeField] protected VectorLayer _vectorData = new();
		...
		[SerializeField] protected EditorPreviewOptions _previewOptions = new();
		...
		{
			get => _previewOptions.isPreviewEnabled;
		...
		public AbstractMapVisualizer MapVisualizer
		{
			get
			{
				if (_mapVisualizer == null) _mapVisualizer = ScriptableObject.CreateInstance<MapVisualizer>();
		...
		public AbstractTileProvider TileProvider
		{
			...
			set
			{
				if (_tileProvider != null) _tileProvider.ExtentChanged -= OnMapExtentChanged;
				_tileProvider = value;
				if (_tileProvider != null) _tileProvider.ExtentChanged += OnMapExtentChanged;
			}
		}
		...
		public virtual void Initialize(Vector2d latLon, int zoom)
		{
			...
			if (_options == null) _options = new MapOptions();
			_options.locationOptions.latitudeLongitude =
				string.Format(CultureInfo.InvariantCulture, "{0},{1}", latLon.x, latLon.y);
		...
		{
			if (Application.isEditor && !Application.isPlaying && IsEditorPreviewEnabled == false) return;
			if (TileProvider != null) TileProvider.UpdateTileProvider();
		}
		...
		{
			if (Application.isEditor && !Application.isPlaying && !IsEditorPreviewEnabled) return;

			//so map will be snapped to zero using next new tile loaded
			_worldHeightFixed = false;
			var differenceInZoom = 0.0f;
			var isAtInitialZoom = false;
			// Update map zoom, if it has changed.
			if (Math.Abs(Zoom - zoom) > Utils.Constants.EpsilonFloatingPoint) SetZoom(zoom);

			// Compute difference in zoom. Will be used to calculate correct scale of the map.
			differenceInZoom = Zoom - InitialZoom;
			isAtInitialZoom = differenceInZoom - 0.0 < Utils.Constants.EpsilonFloatingPoint;

			//Update center latitude longitude
			var centerLatitudeLongitude = latLon;
			var xDelta = centerLatitudeLongitude.x;
			var zDelta = centerLatitudeLongitude.y;

			xDelta = xDelta > 0
				? Mathd.Min(xDelta, Utils.Constants.LatitudeMax)
				: Mathd.Max(xDelta, -Utils.Constants.LatitudeMax);
			zDelta = zDelta > 0
				? Mathd.Min(zDelta, Utils.Constants.LongitudeMax)
				: Mathd.Max(zDelta, -Utils.Constants.LongitudeMax);
			...
			if (Math.Abs(differenceInZoom) > Utils.Constants.EpsilonFloatingPoint || isAtInitialZoom)
			{
			...
			//Update Tile extent.
			if (TileProvider != null) TileProvider.UpdateTileExtent();
		...
		public bool IsAccessTokenValid
		{
			get
			{
				...
				try
				{
					...
					if (MapboxAccess.Instance.Configuration == null ||
					    string.IsNullOrEmpty(MapboxAccess.Instance.Configuration.AccessToken)) return false;
		...
		private void OnEnable()
		{
			...
			if (_options.tileMaterial == null) _options.tileMaterial = new Material(Shader.Find("Standard"));

			if (_options.loadingTexture == null) _options.loadingTexture = new Texture2D(1, 1);
		}

		// TODO: implement IDisposable, instead?
		protected virtual void OnDestroy()
		{
			if (TileProvider != null) TileProvider.ExtentChanged -= OnMapExtentChanged;
		...
		{
			var destroyChildStartIndex = transform.childCount - 1;
			for (var i = destroyChildStartIndex; i >= 0; i--) transform.GetChild(i).gameObject.Destroy();
		}
		...
		private void MapOnStartRoutine(bool coroutine = true)
		{
			...
			{
				if (coroutine) StartCoroutine("SetupAccess");
		...
		public void EnableEditorPreview()
		{
			...
			SetUpMap();
			if (OnEditorPreviewEnabled != null) OnEditorPreviewEnabled();
		}
		...
		public void DisableEditorPreview()
		{
			...
			if (_mapVisualizer != null) _mapVisualizer.ClearMap();
			DestroyTileProvider();

			if (OnEditorPreviewDisabled != null) OnEditorPreviewDisabled();
		...
		protected virtual void SetUpMap()
		{
			...
			if (_imagery == null) _imagery = new ImageryLayer();
			_imagery.Initialize();

			if (_terrain == null) _terrain = new TerrainLayer();
			_terrain.Initialize();

			if (_vectorData == null) _vectorData = new VectorLayer();
			_vectorData.Initialize();

			_mapVisualizer.Factories = new List<AbstractTileFactory>
			{
				_terrain.Factory, _imagery.Factory, _vectorData.Factory
			};
		...
		protected virtual void InitializeMap(MapOptions options)
		{
			...
			_options.locationOptions.PropertyHasChanged += (sender, eventArgs) => { UpdateMap(); };

			_options.extentOptions.PropertyHasChanged += (sender, eventArgs) => { OnTileProviderChanged(); };

			_options.extentOptions.defaultExtents.PropertyHasChanged += (sender, eventArgs) =>
			{
			...
			_options.placementOptions.PropertyHasChanged += (sender, eventArgs) =>
			{
				SetPlacementStrategy();
				UpdateMap();
			};

			_options.scalingOptions.PropertyHasChanged += (sender, eventArgs) =>
			{
				SetScalingStrategy();
				UpdateMap();
			};
		...
		private void SetTileProvider()
		{
			if (_options.extentOptions.extentType != MapExtentType.Custom)
			{
				...
				var tileProviderName = "TileProvider";
		...
		private void TriggerTileRedrawForExtent(ExtentArgs currentExtent)
		{
			...
			if (tilesToProcess.Count > 0)
			{
				...
				foreach (var t2r in tilesToProcess) TileProvider_OnTileRemoved(t2r);
			}
		...
		private void OnImageOrTerrainUpdateLayer(object sender, EventArgs eventArgs)
		{
			...
			if (layerUpdateArgs != null)
			{
				...
				if (layerUpdateArgs.effectsVectorLayer) RedrawVectorDataLayer();
				OnMapRedrawn();
			}
		}
		...
		private void OnVectorDataSubLayerRemoved(object sender, EventArgs eventArgs)
		{
			...
			if (layerUpdateArgs.visualizer != null)
				_mapVisualizer.RemoveTilesFromLayer((VectorTileFactory)layerUpdateArgs.factory,
		...
		private void OnVectorDataUpdateLayer(object sender, EventArgs eventArgs)
		{
			var layerUpdateArgs = eventArgs as VectorLayerUpdateArgs;
			...
			if (layerUpdateArgs.visualizer != null)
				...
				_mapVisualizer.UnregisterAndRedrawTilesFromLayer((VectorTileFactory)layerUpdateArgs.factory,
					layerUpdateArgs.visualizer);
			else
		...
		private void OnTileProviderChanged()
		{
			...
			SetTileProvider();
			TileProvider.Initialize(this);
			if (IsEditorPreviewEnabled) TileProvider.UpdateTileExtent();
		}
		...
		private Vector3 GeoToWorldPositionXZ(Vector2d latitudeLongitude)
		{
			...
			var worldPos = Conversions
				.GeoToWorldPosition(latitudeLongitude, CenterMercator, WorldRelativeScale * scaleFactor).ToVector3xz();
		...
		protected virtual float QueryElevationAtInternal(Vector2d latlong, out float tileScale)
		{
			...
			var foundTile =
				MapVisualizer.ActiveTiles.TryGetValue(
					Conversions.LatitudeLongitudeToTileId(latlong.x, latlong.y, (int)Zoom), out tile);
			if (foundTile)
			{
				tileScale = tile.TileScale;
				var _rect = tile.Rect;
				return tile.QueryHeightData((float)((_meters - _rect.Min).x / _rect.Size.x),
		...
		public virtual Vector2d WorldToGeoPosition(Vector3 realworldPoint)
		{
			...
			return Root.InverseTransformPoint(realworldPoint)
				.GetGeoPosition(CenterMercator, WorldRelativeScale * scaleFactor);
		}
		...
		{
			var tileScale = 1f;
			var height = QueryElevationAtInternal(latlong, out tileScale);
		...
		public virtual void SetExtent(MapExtentType extentType, ExtentOptions extentOptions = null)
		{
			...
			if (extentOptions != null)
			{
				...
				if (currentOptions.GetType() == extentOptions.GetType()) currentOptions = extentOptions;
			}
		...
		///     Use <paramref name="placementType" /> = <c> MapPlacementType.AtTileCenter</c> to place map root at the center of
		///     tile containing the latitude,longitude.
		///     Use <paramref name="placementType" /> = <c> MapPlacementType.AtLocationCenter</c> to place map root at the
		///     latitude,longitude.
		/// </summary>
