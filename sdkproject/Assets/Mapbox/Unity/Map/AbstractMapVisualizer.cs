...
namespace Mapbox.Unity.Map
{
	/// <summary>
	///     Map Visualizer
	///     Represents a map.Doesn’t contain much logic and at the moment, it creates requested tiles and relays them to the
	///     factories
	...
	public abstract class AbstractMapVisualizer : ScriptableObject
	{
		...
		public virtual void Destroy()
		{
			...
			foreach (var tileId in _activeTiles.Keys.ToList()) DisposeTile(tileId);
		...
		public virtual UnityTile LoadTile(UnwrappedTileId tileId)
		{
			...
			if (_inactiveTiles.Count > 0) unityTile = _inactiveTiles.Dequeue();
			...
			foreach (var factory in Factories) factory.Register(unityTile);
		...
		public virtual void DisposeTile(UnwrappedTileId tileId)
		{
			...
			foreach (var factory in Factories) factory.Unregister(unityTile);
		...
		public virtual void RepositionTile(UnwrappedTileId tileId)
		{
			...
			if (ActiveTiles.TryGetValue(tileId, out currentTile)) PlaceTile(tileId, currentTile, _map);
		}
		...
		{
			foreach (var tileBundle in _activeTiles) factory.Unregister(tileBundle.Value);
		}

		public void UnregisterAndRedrawTilesFromLayer(VectorTileFactory factory, LayerVisualizerBase layerVisualizer)
		{
			foreach (var tileBundle in _activeTiles) factory.UnregisterLayer(tileBundle.Value, layerVisualizer);
			...
			foreach (var tileBundle in _activeTiles) factory.RedrawSubLayer(tileBundle.Value, layerVisualizer);
		}
		...
		{
			foreach (var tileBundle in _activeTiles) factory.Register(tileBundle.Value);
		}

		public void UpdateTileForProperty(AbstractTileFactory factory, LayerUpdateArgs updateArgs)
		{
			foreach (var tileBundle in _activeTiles) factory.UpdateTileProperty(tileBundle.Value, updateArgs);
		}
		...
		{
			if (tile.HeightDataState == TilePropertyState.Loaded) OnTileHeightProcessingFinished(tile);
			TileStateChanged(tile);
		}

		private void TileRasterStateChanged(UnityTile tile)
		{
			if (tile.RasterDataState == TilePropertyState.Loaded) OnTileImageProcessingFinished(tile);
			TileStateChanged(tile);
		}

		private void TileVectorStateChanged(UnityTile tile)
		{
			if (tile.VectorDataState == TilePropertyState.Loaded) OnTileVectorProcessingFinished(tile);
			TileStateChanged(tile);
		}
		...
		///     Returns a <see cref="T:Mapbox.Map.TileErrorEventArgs" /> instance as a parameter, for the tile on which error
		///     occurred.
		/// </summary>
