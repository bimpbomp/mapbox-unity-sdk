...
namespace KDTree
{
	...
	public class IntervalHeap<T>
	{
		...
		private void SiftDownMin(int iParent)
		{
			...
			for (var iChild = iParent * 2 + 2; iChild < Size; iParent = iChild, iChild = iParent * 2 + 2)
			{
				// If the next child is less than the current child, select the next one.
				if (iChild + 2 < Size && tKeys[iChild + 2] < tKeys[iChild]) iChild += 2;
				...
				if (tKeys[iChild] < tKeys[iParent])
				{
					...
					if (iChild + 1 < Size && tKeys[iChild + 1] < tKeys[iChild]) Swap(iChild, iChild + 1);
				}
				else
				{
					break;
		...
		private void SiftDownMax(int iParent)
		{
			...
			for (var iChild = iParent * 2 + 1; iChild <= Size; iParent = iChild, iChild = iParent * 2 + 1)
			{
				...
				else if (iChild + 2 < Size)
				{
					...
					if (tKeys[iChild + 2] > tKeys[iChild]) iChild += 2;
				}
				...
				if (tKeys[iChild] > tKeys[iParent])
				{
					Swap(iParent, iChild);
					// Swap with pair if necessary
					if (tKeys[iChild - 1] > tKeys[iChild]) Swap(iChild, iChild - 1);
				}
				else
				{
					break;
