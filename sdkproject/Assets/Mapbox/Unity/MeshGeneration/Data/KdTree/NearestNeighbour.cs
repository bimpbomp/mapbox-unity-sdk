...
namespace KDTree
{
	...
	public class NearestNeighbour<T> : IEnumerator<T>, IEnumerable<T>
	{
		...
		private readonly DistanceFunctions kDistanceFunction;
		...
		private readonly double[] tSearchPoint;
		...
		///     applied.
		/// </param>
		public NearestNeighbour(KDNode<T> pRoot, double[] tSearchPoint, DistanceFunctions kDistance, int iMaxPoints,
			double fThreshold)
		{
		...
		public bool MoveNext()
		{
			...
			while (pPending.Size > 0 && (pEvaluated.Size == 0 || pPending.MinKey < pEvaluated.MinKey))
			{
				...
				while (!pCursor.IsLeaf)
				{
					...
					var fDistance =
						kDistanceFunction.DistanceToRectangle(tSearchPoint, pNotTaken.tMinBound, pNotTaken.tMaxBound);

					// If it is greater than the threshold, skip.
					if (fThreshold >= 0 && fDistance > fThreshold) continue;
