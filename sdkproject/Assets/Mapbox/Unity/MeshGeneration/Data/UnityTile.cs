...
namespace Mapbox.Unity.MeshGeneration.Data
{
	public class UnityTile : MonoBehaviour
	{
		...
		[SerializeField] private TilePropertyState _rasterDataState;

		[SerializeField] private TilePropertyState _heightDataState;

		[SerializeField] private TilePropertyState _vectorDataState;
		...
		public TilePropertyState TileState
		{
			...
			set
			{
				if (_tileState != value) _tileState = value;
			}
		}
		...
		{
			Cancel();
			if (_heightTexture != null) _heightTexture.Destroy();
			if (_rasterData != null) _rasterData.Destroy();
		}
			...
			bool addCollider = false)
		{
			if (HeightDataState != TilePropertyState.Unregistered)
			{
				...
				if (_heightTexture == null) _heightTexture = new Texture2D(0, 0);
				...
				if (HeightData == null) HeightData = new float[256 * 256];
				...
				for (var yy = 0; yy < 256; ++yy)
				{
					...
					HeightData[xx * 256 + yy] = relativeScale * heightMultiplier *
		...
		{
			if (VectorDataState != TilePropertyState.Unregistered) VectorData = vectorTile;
		}
		...
		{
			if (HeightData != null) return HeightData[(int)(y * 255) * 256 + (int)(x * 255)] * _tileScale;
		...
		public void Cancel()
		{
			for (int i = 0, _tilesCount = _tiles.Count; i < _tilesCount; i++) _tiles[i].Cancel();
		}
