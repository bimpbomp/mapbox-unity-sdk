	...
	public class FlatTerrainStrategy : TerrainStrategy
	{
		...
		public override void RegisterTile(UnityTile tile)
		{
			...
			if (_elevationOptions.colliderOptions.addCollider)
			{
				...
				if (meshCollider) meshCollider.sharedMesh = tile.MeshFilter.sharedMesh;
			}
		}
		...
		private void BuildQuad(UnityTile tile)
		{
			...
			verts[1] = tile.TileScale * new Vector3((float)(tile.Rect.Max.x - tile.Rect.Center.x), 0,
				(float)(tile.Rect.Min.y - tile.Rect.Center.y));
			verts[2] = tile.TileScale * (tile.Rect.Max - tile.Rect.Center).ToVector3xz();
			verts[3] = tile.TileScale * new Vector3((float)(tile.Rect.Min.x - tile.Rect.Center.x), 0,
			...
			var uvlist = new Vector2[4] { new(0, 1), new(1, 1), new(1, 0), new(0, 0) };
			unityMesh.uv = uvlist;
			_cachedQuad = new MeshDataArray { Vertices = verts, Normals = norms, Triangles = trilist, Uvs = uvlist };
		}
		...
		private void BuildQuadWithSides(UnityTile tile)
		{
			...
			var norm = Constants.Math.Vector3Up;
			for (var i = 0; i < 4; i++)
			{
				verts[4 * (i + 1)] = verts[i];
				...
				norm = Vector3.Cross(verts[4 * (i + 1) + 1] - verts[4 * (i + 1) + 2],
					verts[4 * (i + 1)] - verts[4 * (i + 1) + 1]).normalized;
				norms[4 * (i + 1)] = norm;
				norms[4 * (i + 1) + 1] = norm;
				norms[4 * (i + 1) + 2] = norm;
				norms[4 * (i + 1) + 3] = norm;
			...
			{
				Vertices = verts, Normals = norms, Triangles = trilist.ToArray(), Uvs = uvlist
			};
		}
	}
}
