...
namespace Mapbox.Unity.MeshGeneration.Factories
{
	...
	///     pull whole vector data and you can use 'Style Optimized Vector Tile Factory' to pull only the layer you want to
	///     use.
	/// </summary>
	...
	public class VectorTileFactory : AbstractTileFactory
	{
		...
		{
			get => Properties.sourceOptions.Id;

			set => Properties.sourceOptions.Id = value;
		}

		public VectorLayerProperties Properties { get; private set; }
		...
		{
			if (_layerProgress.ContainsKey(tile)) _layerProgress.Remove(tile);
			if (_tilesWaitingProcessing.Contains(tile)) _tilesWaitingProcessing.Remove(tile);

			if (visualizer != null) visualizer.UnregisterTile(tile);
		}
		...
		public virtual LayerVisualizerBase AddVectorLayerVisualizer(VectorSubLayerProperties subLayer)
		{
			...
			var isPrimitiveTypeValidForBuidingIds = subLayer.coreOptions.geometryType == VectorPrimitiveType.Polygon ||
			                                        subLayer.coreOptions.geometryType == VectorPrimitiveType.Custom;
			var isSourceValidForBuildingIds = Properties.sourceType != VectorSourceType.MapboxStreets;
		...
		public virtual LayerVisualizerBase AddPOIVectorLayerVisualizer(PrefabItemOptions poiSubLayer)
		{
			...
			poiSubLayer.performanceOptions = Properties.performanceOptions;
			((LocationPrefabsLayerVisualizer)visualizer).SetProperties(poiSubLayer);
		...
		public virtual void RemoveVectorLayerVisualizer(LayerVisualizerBase subLayer)
		{
			...
			if (_layerBuilder.ContainsKey(subLayer.Key))
			{
				...
				else if (subLayer.SubLayerProperties is PrefabItemOptions &&
				         Properties.locationPrefabList.Contains(subLayer.SubLayerProperties as PrefabItemOptions))
		...
		{
			if (string.IsNullOrEmpty(TilesetId) || Properties.sourceOptions.isActive == false ||
		...
		{
			if (_layerProgress != null && _layerProgress.ContainsKey(tile)) _layerProgress.Remove(tile);
		...
		public override void UpdateTileProperty(UnityTile tile, LayerUpdateArgs updateArgs)
		{
			...
			if (updateArgs.property.NeedsForceUpdate()) Unregister(tile);
			Register(tile);
		}
		...
		private void CreateMeshes(UnityTile tile)
		{
			...
			for (var i = 0; i < nameList.Count; i++) CreateFeatureWithBuilder(tile, nameList[i], builderList[i]);
			...
			for (var i = 0; i < builderList.Count; i++) CreateFeatureWithBuilder(tile, emptyLayer, builderList[i]);
		...
		private void CreateFeatureWithBuilder(UnityTile tile, string layerName, LayerVisualizerBase builder)
		{
			if (builder.Active)
			{
				...
				if (layerName != "")
					...
					builder.Create(tile.VectorData.Data.GetLayer(tile.VectorData.Data.LayerNames()[0]), tile,
		...
		{
			foreach (var item in Properties.locationPrefabList) AddPOIVectorLayerVisualizer(item);
		}

		private void CreateLayerVisualizers()
		{
			foreach (var sublayer in Properties.vectorSubLayers) AddVectorLayerVisualizer(sublayer);
		}
