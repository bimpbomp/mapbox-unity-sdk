...
namespace Mapbox.Unity.MeshGeneration.Filters
{
	public class TypeFilter : FilterBase
	{
		...
		[SerializeField] private TypeFilterType _behaviour;
	...
	public class LayerFilterComparer : ILayerFeatureFilterComparer
	{
		...
		{
			return new LayerFilterCombiner { Type = LayerFilterCombinerOperationType.Any, Filters = filters.ToList() };
		}

		public static ILayerFeatureFilterComparer AllOf(params ILayerFeatureFilterComparer[] filters)
		{
			return new LayerFilterCombiner { Type = LayerFilterCombinerOperationType.All, Filters = filters.ToList() };
		}

		public static ILayerFeatureFilterComparer NoneOf(params ILayerFeatureFilterComparer[] filters)
		{
			return new LayerFilterCombiner { Type = LayerFilterCombinerOperationType.None, Filters = filters.ToList() };
		}
		...
		{
			return new LayerPropertyInRangeFilterComparer { Key = property, Min = min, Max = max };
		}
		...
		{
			return new LayerPropertyContainsFilterComparer { Key = property, ValueSet = values.ToList() };
		}
	}
	...
	public class LayerHasPropertyFilterComparer : ILayerFeatureFilterComparer
	{
		...
		{
			object property;
			if (feature.Properties.TryGetValue(Key, out property)) return PropertyComparer(property);
			return false;
		}
	...
	public class LayerPropertyInRangeFilterComparer : LayerHasPropertyFilterComparer
	{
		...
		protected override bool PropertyComparer(object property)
		{
			...
			if (propertyValue < Min) return false;
			if (propertyValue >= Max) return false;
			return true;
		}
	}
	...
	public class LayerPropertyIsGreaterFilterComparer : LayerHasPropertyFilterComparer
	{
		...
		protected override bool PropertyComparer(object property)
		{
			var propertyValue = Convert.ToDouble(property);
			if (property == null) return false;
			if (propertyValue > Min) return true;
			return false;
		}
	}
	...
	public class LayerPropertyIsEqualFilterComparer : LayerHasPropertyFilterComparer
	{
		...
		protected override bool PropertyComparer(object property)
		{
			...
			var propertyValue = Convert.ToDouble(property);
			if (Math.Abs(propertyValue - Min) < Utils.Constants.EpsilonFloatingPoint) return true;
			return false;
		}
	}
	...
	public class LayerFilter : MapboxDataProperty, ILayerFilter
	{
		...
		[SerializeField] [Tooltip("Description of the property defined as key.")]
		...
		{
			if (_delimiters == null) _delimiters = new[] { ',' };
