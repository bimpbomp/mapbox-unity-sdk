...
{
	/// <summary>
	///     Merged Modifier Stack, just like regular Modifier stack, creates a game object from features. But the difference
	///     is, regular modifier stack creates a game object for each given faeture meanwhile Merged Modifier Stack merges
	///     meshes and creates one game object for all features (until the 65k vertex limit).
	///     It has extremely higher performance compared to regular modifier stack but since it merged all entities together,
	///     it also loses all individual entity data & makes it harder to interact with them.
	...
	{
		private readonly Dictionary<UnityTile, List<VectorEntity>> _activeObjects = new();
		private readonly Dictionary<UnityTile, int> _buildingCount = new();
		private readonly Dictionary<UnityTile, List<MeshData>> _cached = new();
		private readonly Dictionary<UnityTile, int> _cacheVertexCount = new();
		...
		public override void OnUnregisterTile(UnityTile tile)
		{
			...
			if (_activeObjects.ContainsKey(tile))
			{
				...
				{
					if (null != _activeObjects[tile][i].GameObject) _activeObjects[tile][i].GameObject.SetActive(false);
			...
			//reset all counters
			if (_cacheVertexCount.ContainsKey(tile)) _cacheVertexCount.Remove(tile);
			...
			if (_buildingCount.ContainsKey(tile)) _buildingCount.Remove(tile);
		}
		...
		public override void Initialize()
		{
			...
			for (var i = 0; i < _counter; i++) MeshModifiers[i].Initialize();

			_counter = GoModifiers.Count;
			for (var i = 0; i < _counter; i++) GoModifiers[i].Initialize();
		}

		public override GameObject Execute(UnityTile tile, VectorFeatureUnity feature, MeshData meshData,
		...
		public GameObject End(UnityTile tile, GameObject parent, string name = "")
		{
			...
			if (_cached.ContainsKey(tile))
			{
				...
				for (var i = 0; i < _counter; i++)
				{
					...
					c2 = _temp2MeshData.UV.Count;
					for (var j = 0; j < c2; j++) _tempMeshData.UV[j].AddRange(_temp2MeshData.UV[j]);
				...
				if (_tempMeshData.Vertices.Count > 3)
				{
					...
					for (var i = 0; i < _counter; i++) _tempVectorEntity.Mesh.SetUVs(i, _tempMeshData.UV[i]);

					_tempVectorEntity.GameObject.transform.SetParent(tile.transform, false);

					if (!_activeObjects.ContainsKey(tile)) _activeObjects.Add(tile, _listPool.GetObject());
		...
		public override void Clear()
		{
			...
			{
				if (vectorEntity.Mesh != null) vectorEntity.Mesh.Destroy(true);
