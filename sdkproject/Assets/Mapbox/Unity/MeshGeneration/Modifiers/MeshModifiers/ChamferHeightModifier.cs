...
namespace Mapbox.Unity.MeshGeneration.Modifiers
{
	...
	public class ChamferHeightModifier : MeshModifier
	{
		...
		[SerializeField] [Tooltip("Fixed height value for ForceHeight option")]
		...
		[SerializeField] [Range(0.1f, 2)] [Tooltip("Chamfer width value")]
		...
		public override void Run(VectorFeatureUnity feature, MeshData md, UnityTile tile = null)
		{
			...
			if (!_forceHeight) GetHeightData(feature, tile.TileScale, ref minHeight, ref hf);
		...
		private void Sides(VectorFeatureUnity feature, MeshData meshData, float hf, int originalVertexCount)
		{
			var d = 0f;
			Vector3 v1;
			var v2 = Constants.Math.Vector3Zero;
			var ind = 0;

			var wallTri = new List<int>();
			var wallUv = new List<Vector2>();
			meshData.Vertices.Add(new Vector3(meshData.Vertices[originalVertexCount - 1].x,
				meshData.Vertices[originalVertexCount - 1].y - hf, meshData.Vertices[originalVertexCount - 1].z));
			...
			for (var i = 0; i < meshData.Edges.Count; i += 2)
			{
		...
		private static void FlattenTops(MeshData meshData, float minHeight, ref float hf, ref float max, ref float min)
		{
			...
			for (var i = 0; i < meshData.Vertices.Count; i++)
				meshData.Vertices[i] =
					new Vector3(meshData.Vertices[i].x, max + minHeight + hf, meshData.Vertices[i].z);
			hf += max - min;
		}
		...
		public void Chamfer(VectorFeatureUnity feature, MeshData md, UnityTile tile = null)
		{
			...
			for (var t = 0; t < md.Triangles[0].Count; t++) md.Triangles[0][t] *= 3;
			...
			for (var i = 0; i < feature.Points.Count; i++)
			{
				...
				for (var j = 0; j < count; j++)
				{
					...
					pij1 = new Vector3(
						md.Vertices[next].x + n1.x, 0,
						md.Vertices[next].z + n1.z);
					pij2 = new Vector3(
						md.Vertices[current].x + n1.x, 0,
						md.Vertices[current].z + n1.z);
					...
					pjk1 = new Vector3(
						md.Vertices[current].x + n2.x, 0,
						md.Vertices[current].z + n2.z);
					pjk2 = new Vector3(
						md.Vertices[prev].x + n2.x, 0,
						md.Vertices[prev].z + n2.z);
		...
		{
			var enlarged_points = new List<Vector3>();
			var num_points = old_points.Count;
			for (var j = 0; j < num_points; j++)
			{
				...
				v1.Normalize();
				v1 *= offset;
				var n1 = new Vector3(-v1.z, 0, v1.x);

				var pij1 = new Vector3(
					old_points[i].x + n1.x, 0,
					old_points[i].z + n1.z);
				var pij2 = new Vector3(
					old_points[j].x + n1.x, 0,
					old_points[j].z + n1.z);
				...
				v2.Normalize();
				v2 *= offset;
				var n2 = new Vector3(-v2.z, 0, v2.x);

				var pjk1 = new Vector3(
					old_points[j].x + n2.x, 0,
					old_points[j].z + n2.z);
				var pjk2 = new Vector3(
					old_points[k].x + n2.x, 0,
					old_points[k].z + n2.z);
