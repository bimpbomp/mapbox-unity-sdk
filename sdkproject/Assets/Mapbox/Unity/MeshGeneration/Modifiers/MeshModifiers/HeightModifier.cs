...
namespace Mapbox.Unity.MeshGeneration.Modifiers
{
	public class MinMaxPair
	{
		...
		public static MinMaxPair GetMinMaxHeight(List<Vector3> vertices)
		{
			...
			var returnValue = new MinMaxPair { max = float.MinValue, min = float.MaxValue };
	...
	/// <summary>
	///     Height Modifier is responsible for the y axis placement of the feature. It pushes the original vertices upwards by
	///     "height" value and creates side walls around that new polygon down to "min_height" value.
	...
	{
		[SerializeField] [Tooltip("Create side walls as separate submesh.")]
		...
		public override void Run(VectorFeatureUnity feature, MeshData md, UnityTile tile = null)
		{
			...
			if (_options.extrusionGeometryType == ExtrusionGeometryType.SideOnly) md.Triangles[0].Clear();
		...
		protected virtual void QueryHeight(VectorFeatureUnity feature, MeshData md, UnityTile tile, out float maxHeight,
			out float minHeight)
		{
			minHeight = 0.0f;
			maxHeight = 0.0f;
			...
			switch (_options.extrusionType)
			{
				...
				case ExtrusionType.MaxHeight:
					if (feature.Properties.ContainsKey(_options.propertyName))
					{
						...
						{
							Debug.LogError("Property: '" + _options.propertyName +
