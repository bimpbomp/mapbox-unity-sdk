...
namespace Mapbox.Unity.MeshGeneration.Modifiers
{
	...
	/// <summary>
	///     Modifier Stacks
	///     Modifier Stack can be thought as styles as as they contain all the data/settings for how the feature will be
	///     visualized.
	///     They also create the game objects in default implementations in the sdk.
	///     Currently there's two implementations of this; Modifier Stack and Merged Modifier Stack.They work almost exactly
	///     same
	///     (logically) with one difference; modifier stacks creates a game object for each feature while merged modifier
	///     stack,
	///     merges them up as the name suggest and create one game object for multiple(as many as possible) features.Both have
	///     their advantages but the main factor here is the performance.Regular modifier stack creates individual game object
	///     so
	///     it's easier to interact, move, animate etc features.But if you want to visualize whole San Francisco, that would
	///     mean
	...
	public class ModifierStack : ModifierStackBase
	{
		...
		public override void OnUnregisterTile(UnityTile tile)
		{
			if (_activeObjects.ContainsKey(tile))
			{
				...
				{
					foreach (var item in GoModifiers) item.OnPoolItem(_activeObjects[tile][i]);
			...
			GameObject parent = null, string type = "")
		{
			...
			if (moveFeaturePositionTo != PositionTargetType.TileCenter)
			{
				...
				else if (moveFeaturePositionTo == PositionTargetType.CenterOfVertices)
				{
					...
					for (var i = 0; i < _counter; i++)
					{
						...
						for (var j = 0; j < _secondCounter; j++)
						{
				...
				for (var i = 0; i < _counter; i++)
				{
					_secondCounter = feature.Points[i].Count;
					for (var j = 0; j < _secondCounter; j++)
						feature.Points[i][j] = new Vector3(feature.Points[i][j].x - _tempPoint.x, 0,
			...
			if (_tempVectorEntity.GameObject == null) return null;
			...
			if (meshData.Tangents.Count > 0) _tempVectorEntity.Mesh.SetTangents(meshData.Tangents);

			_counter = meshData.Triangles.Count;
			for (var i = 0; i < _counter; i++) _tempVectorEntity.Mesh.SetTriangles(meshData.Triangles[i], i);
			_counter = meshData.UV.Count;
			for (var i = 0; i < _counter; i++) _tempVectorEntity.Mesh.SetUVs(i, meshData.UV[i]);
