...
public class ModelTest : MonoBehaviour
{
	...
	{
		foreach (var layer in _abstractMap.VectorData.GetAllFeatureSubLayers())
			layer.Modeling.ExtrusionOptions.EnableAbsoluteExtrusion(ExtrusionGeometryType.RoofAndSide, 10);
	}
	...
	{
		foreach (var layer in _abstractMap.VectorData.GetAllFeatureSubLayers())
			layer.Modeling.ColliderOptions.SetFeatureCollider((ColliderType)Random.Range(0, 4));
	}
	...
	{
		foreach (var layer in _abstractMap.VectorData.GetAllFeatureSubLayers())
			layer.Modeling.CoreOptions.SetPrimitiveType((VectorPrimitiveType)Random.Range(0, 2));
	}
	...
	{
		foreach (var layer in _abstractMap.VectorData.GetAllLineFeatureSubLayers())
			layer.Modeling.LineOptions.SetJoinType(
				(LineJoinType)(((int)(LineJoinType)layer.lineGeometryOptions.JoinType + 1) % 3));
	}
	...
	{
		foreach (var layer in _abstractMap.VectorData.GetAllLineFeatureSubLayers())
			layer.Modeling.LineOptions.SetCapType(
				(LineCapType)(((int)(LineCapType)layer.lineGeometryOptions.CapType + 1) % 3));
	}
}
