...
namespace Mapbox.Unity.Utilities
{
	...
	public static class Conversions
	{
		...
		public static Vector2d StringToLatLon(string s)
		{
			...
			if (latLonSplit.Length != 2) throw new ArgumentException("Wrong number of arguments");
		...
		/// <param name="lon"> The longitude. </param>
		/// <param name="refPoint">
		///     A <see cref="T:UnityEngine.Vector2d" /> center point to offset resultant xy, this is usually
		///     map's center mercator
		/// </param>
		/// <param name="scale"> Scale in meters. (default scale = 1) </param>
		/// <returns> A <see cref="T:UnityEngine.Vector2d" /> xy tile ID. </returns>
		/// <example>
		///     Converts a Lat/Lon of (37.7749, 122.4194) into Unity coordinates for a map centered at (10,10) and a scale of 2.5
		///     meters for every 1 Unity unit
		///     <code>
		...
		{
			var xPos = radius * Math.Cos(Mathf.Deg2Rad * lat) * Math.Cos(Mathf.Deg2Rad * lon);
			var zPos = radius * Math.Cos(Mathf.Deg2Rad * lat) * Math.Sin(Mathf.Deg2Rad * lon);
			var yPos = radius * Math.Sin(Mathf.Deg2Rad * lat);
		...
		{
			var latitude = Mathf.Asin(point.y / radius);
			var longitude = Mathf.Atan2(point.z, point.x);
		...
		public static RectD TileBounds(Vector2 tileCoordinate, int zoom)
		{
			...
			var max = PixelsToMeters(new Vector2d((tileCoordinate.x + 1) * TileSize, (tileCoordinate.y + 1) * TileSize),
				zoom);
		...
		{
			var min = PixelsToMeters(new Vector2d(unwrappedTileId.X * TileSize, unwrappedTileId.Y * TileSize),
				unwrappedTileId.Z);
			var max = PixelsToMeters(
				new Vector2d((unwrappedTileId.X + 1) * TileSize, (unwrappedTileId.Y + 1) * TileSize),
				unwrappedTileId.Z);
			return new RectD(min, max - min);
		}
		...
		/// <summary>
		///     Get coordinates for a given latitude/longitude in tile-space. Useful when comparing feature geometry to lat/lon
		///     coordinates.
		/// </summary>
		...
		{
			var coordinateTileId = LatitudeLongitudeToTileId(
				coordinate.x, coordinate.y, tileZoom);
			var _meters = LatLonToMeters(coordinate);
			var _rect = TileBounds(coordinateTileId);
		...
		{
			var t = new Vector2((int)Math.Ceiling(p.x / TileSize) - 1, (int)Math.Ceiling(p.y / TileSize) - 1);
			return t;
		}
